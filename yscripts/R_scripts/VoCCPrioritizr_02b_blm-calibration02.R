...
posthoc_marxan <- function(path, outdir, proj.geo, spot) { 
  ...
  # Loop through every scenario-solutionBLM
    # To allocate iterations dataframes
      ...
      for(i in 1:length(dir.scenarios)) {
        # Files location
          out_files <- list.files(path = dir.scenarios[i], pattern = "*cost.*.csv$", full.names = TRUE)
  ...
  system.time(posthoc_marxan(path = "/QRISdata/Q1216/BritoMorales/Project04b/vfinal-sol_figs/ublm-cal_rce-vocc050_targets-mix",
                             outdir = "/QRISdata/Q1216/BritoMorales/Project04b/vfinal-sol_figs/ublm-cal_rce-vocc050_targets-mix/",
