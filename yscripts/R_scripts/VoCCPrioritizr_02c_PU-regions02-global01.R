        ...
        if(length(rce_csv) == 0 & length(vocc_csv) == 0) { # if you have a base scenario just do it without climatic information
          ...
          # Defining targets and creating .csv species targets file
            trg <- file_olayer_species %>%
              ...
              dplyr::mutate(targets = (max_target - ((cells/length(unique(file_olayer_pus$layer))) * (max_target - min_target)))) %>% # relative to global planning region
              dplyr::ungroup()
          ...
          # Defining targets and creating .csv species targets file [BUT NOT WRITING IT UNTIL THE END]
            trg <- file_olayer_species %>%
              dplyr::group_by(feature_names) %>%
              dplyr::summarise(cells = n()) %>% # global cells for that species
              dplyr::mutate(targets = (max_target - ((cells/length(unique(file_olayer_pus$layer))) * (max_target - min_target)))) %>% # relative to global planning region
          ...
          # Creating Species LOW-VoCC/RCE Feature data
            # VoCC
              file_olayer_species_vocc <- rce_vocc %>% 
                ...
                dplyr::arrange(pu) %>% 
                dplyr::group_by(feature_names_prov.x) %>% 
          ...
          # Writing the FINAL FEATURES FILES WITH SPECIES, VOCC25QT, RCE25QT ALL BY PROVINCES
            ...
            features_final <- rbind(file_olayer_species, file_olayer_species_vocc, file_olayer_species_rce)
              ns1 <- unlist(strsplit(x = basename(dir.scenarios[j]), split = "_"))[2]
          ...
          # Writing the FINAL TARGET FILE WITH SPECIES, VOCC25QT, RCE25QT ALL BY PROVINCES 
            # targets_final <- rbind(global_trg, target_vocc_final, target_rce_final, target_vocc_species_final, target_rce_species_final)
            targets_final <- rbind(global_trg, target_vocc_species_final, target_rce_species_final)
          ...
          # Creating Species LOW-VoCC Feature data
            ...
            # Writing the FINAL FEATURES FILES WITH SPECIES, VOCC25QT, ALL BY PROVINCES
              ...
              features_final <- rbind(file_olayer_species, file_olayer_species_vocc)
              ns1 <- unlist(strsplit(x = basename(dir.scenarios[j]), split = "_"))[2]
          ...
          # Targets Low VoCC-Species
            ...
            # Writing the FINAL TARGET FILE WITH SPECIES, VOCC25QT, RCE25QT ALL BY PROVINCES 
              # targets_final <- rbind(global_trg, target_vocc_final, target_vocc_species_final)
              targets_final <- rbind(global_trg, target_vocc_species_final)
          ...
          # Creating Species LOW-RCE Feature data
            ...
            # Writing the FINAL FEATURES FILES WITH SPECIES, RCE25QT ALL BY PROVINCES
              ...
              features_final <- rbind(file_olayer_species, file_olayer_species_rce)
              ns1 <- unlist(strsplit(x = basename(dir.scenarios[j]), split = "_"))[2]
          ...
          # Targets Low RCE-Species
            ...
            # Writing the FINAL TARGET FILE WITH SPECIES, RCE25QT ALL BY PROVINCES 
              # targets_final <- rbind(global_trg, target_rce_final, target_rce_species_final)
              targets_final <- rbind(global_trg, target_rce_species_final)
