...
general_plots <- function(path, outdir, figure) { 
  ...
  # Getting the plan to use lappy future parallel package
    plan(multiprocess)
    ...
    if(figure == "general") { 
      # Axis legend
        cost_y_axis <- c("Epipelagic", "Mesopelagic", "Bathyabyssopelagic")
        cost_main_tittles <- c("Opportunity Cost", "", "")
        rich_y_axis <- c("", "", "")
        rich_main_tittles <- c("Richness", "", "")
      ...
      # Creating PLOTS Species Provinces
        ...
        # Defining themes
          theme_opts3 <- list(theme(panel.grid.minor = element_blank(),
                                    ...
                                    legend.title = element_text(colour = "black", face = "bold", size = 17),
        ...
        # Plotting the figures
          p <- ggplot() +
            ...
            scale_fill_gradientn(name = "RCE index",
                                 ...
                                 breaks = seq(1, 11, 1),
                                 labels = cv_rce) +
            ggtitle(main_tittles[indexing]) +
            labs(y = y_axis[indexing]) +
            theme_opts3})
