    ...
    # Loop
      summary_list <- foreach(j = 1:length(dir.scenarios), .packages = c("sf", "raster", "dplyr", "lwgeom", "stringr", "magrittr")) %dopar% { 
        # Files location
          ...
          mpas_csv <- list.files(path = dir.scenarios[[j]], pattern = "*_mpas.*.csv$", full.names = TRUE)[1]
          vmes_csv <- list.files(path = dir.scenarios[[j]], pattern = "*_VMEs.*.csv$", full.names = TRUE)[1]
        ...
        if(length(rce_csv) != 0 | length(vocc_csv) != 0) {
          ...
          for(i in 1:length(out_files)) {
            # Read outfiles
              ...
              mpas <- read.csv(mpas_csv[i]) %>% 
                dplyr::filter(province != "non-categ_mpas")
              vmes <- read.csv(vmes_csv[i]) %>% 
            ...
            # Extract from the shapefile the object dt
              ...
              for(k in 1:ncol(dt_solutions)) {
                ...
                dt3 <- left_join(x = dt_shp, y = dt2, by = "id") %>% 
                  na.omit() %>% 
                  dplyr::filter(!id %in% unique(c(mpas$layer, vmes$layer)))
                dt4 <- dt3 %>% 
...
system.time(summary_priotitizr(path = "/QRISdata/Q1216/BritoMorales/Project04b/vfinal-sol_sum-stat_03/ublm-cal_1050rce-vocc10100_targets-mix_rawcost_noduplicates",
                               outdir = "/QRISdata/Q1216/BritoMorales/Project04b/vfinal-sol_sum-stat_03/ublm-cal_1050rce-vocc10100_targets-mix_rawcost_noduplicates/",
