...
pu_by_provinces <- function(pu_file, province_file, prov_name, olayer, proj.geo, outdir) {
  ...
  if(prov_name == "Longhurst") {
    ...
    # Get the indicator for the provinces
      ...
      prov_list <- vector("list", length = length(prov_code)) # to allocate results
      prov_par <- foreach(i = 1:length(prov_code), .packages = c("raster", "sf", "data.table", "dplyr")) %dopar% {
        single <- bioprovince %>% 
          filter(ProvCode == prov_code[1])
        nr <- st_nearest_feature(pu_region, bioprovince)
      }
      
      nr <- st_nearest_feature(pu_region[1:10,], bioprovince)
      sf <- pu_region[1:10,] %>% 
        dplyr::mutate(province = paste(as.vector(bioprovince$ProvCode[nr]), prov_name, sep = "_"))
...
system.time(pu_by_provinces(pu_file = "Output/02_abnjs_filterdepth/abnj_02-epipelagic_global_moll_05deg_depth/abnj_02-epipelagic_global_moll_05deg_depth.shp",
                            province_file = "Inputs/Boundaries/LonghurstProvinces/Longhurst_world_v4_2010.shp",
                            prov_name = "Longhurst",
                            olayer = "epipelagic",
